...
class Parser:
    ...
    def parse_comments(self, soup, title, table_name, thread_id=0, thread_activity=None):
        ...
        for a in articles:
            try:
                ...
                try:
                    rank = self.clean_rank(userdetails.find("div")["class"][1])
                except Exception:
                    rank = None

                try:
                    n_stars = len(userdetails.find("div").text)
                except Exception:
                    n_stars = None
                ...
                try:
                    quote_host = str([i["data-host"] for i in a.select(selectors["code_block"])])
                except Exception:
                    quote_host = None
                try:
                    quote_url = str([i["data-url"] for i in a.select(selectors["code_block"])])
                except Exception:
                    quote_url = None
                try:
    ...
    def scrape_op(self, soup_block, thread_activity=None):
        try:
            ...
            try:
                user_title = userinfo.find("span", class_="userTitle").text
            except Exception:
                user_title = None
            ...
            try:
                quote_host = str([i["data-host"] for i in soup_block.select(selectors["code_block"])])
            except Exception:
                quote_host = None
            try:
                quote_url = str([i["data-url"] for i in soup_block.select(selectors["code_block"])])
            except Exception:
                quote_url = None
            try:
